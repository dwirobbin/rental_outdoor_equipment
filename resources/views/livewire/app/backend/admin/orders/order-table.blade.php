        ...
        <div class="text-muted my-1 my-sm-0">
            <div class="dropdown-center btn-group">
                <button class="btn btn-info dropdown-toggle arrow-none" data-bs-toggle="dropdown" @disabled($bulkSelectedDisabled)
                    ...
                    <span wire:loading.remove wire:target="confirm-delete-selected-order">
                        Terpilih ({{ count($bulkSelected) }}) Data

                        <i class="mdi mdi-chevron-down"></i>
                    </span>

                    <span wire:loading wire:target="confirm-delete-selected-order" class="spinner-border spinner-border-sm"></span>
                    <span wire:loading wire:target="confirm-delete-selected-order" role="status">Loading..</span>
                </button>

                <ul class="dropdown-menu dropdown-menu-animated">
                    <li>
                        @can('delete', \App\Models\Order::class)
                            <button x-on:click="$dispatch('confirm-delete-selected-order')" class="dropdown-item">
                                Hapus
                            </button>
                        @endcan
                    </li>
        ...
        <table class="table table-centered table-nowrap table-sortable">
            <thead class="table-light ">
                <tr>
                    ...
                    <th class="sort @if ($sortColumn == 'penalty') {{ $sortDirection }} @endif"
                        x-on:click="$dispatch('sort', { columnName: 'penalty' })">
                        Denda
                    </th>
            ...
            <tbody>
                @forelse ($orders as $key => $order)
                    <tr class="align-middle" wire:key='{{ $order->id }}'>
                        ...
                        <td class="font-15" style="min-width: 142px;">{{ $order->orderedBy->name }}</td>
                        <td class="font-15">
                        ...
                        <td class="table-action">
                            @can('create', \App\Models\Order::class)
                                <a x-on:click="$dispatch('set-admin-chat-order-data', { id: {{ $order->id }} })"
                                    class="action-icon position-relative" role="button" data-bs-toggle="modal"
                                    data-bs-target="#admin-chat-order">
                                    <i class="uil-comments-alt noti-icon"></i>
                                </a>
                            @endcan
                            @can('view', $order)
                                <a href="{{ route('admin_area.order.detail', ['orderCode' => $order->order_code]) }}" class="action-icon">
                                    <i class="mdi mdi-eye"></i>
                                </a>
                            @endcan
                            @can('update', $order)
                                <a x-on:click="$dispatch('set-order-data', { id: {{ $order->id }} })" class="action-icon" role="button"
                                    data-bs-toggle="modal" data-bs-target="#edit-order-status">
                                    <i class="mdi mdi-square-edit-outline"></i>
                                </a>
                            @endcan
                            @can('delete', $order)
                                <a x-on:click="confirmDeleteOrder({{ $order->id }}, '{{ $order->order_code }}')" class="action-icon"
                                    role="button">
