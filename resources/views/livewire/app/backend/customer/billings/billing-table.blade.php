            ...
            <tbody>
                @forelse ($billings as $key => $billing)
                    <tr class="align-middle" wire:key='{{ $billing->id }}'>
                        ...
                        <td class="font-15">{{ $billing->due_date->translatedFormat('d M Y') }}</td>
                        ...
                        <td class="table-action">
                            @unless (auth()->user()->can('view', $billing))
                                <a href="{{ route('customer_area.billing.detail', ['orderCode' => $billing->order->order_code]) }}"
                                    class="action-icon">
                                    <i class="mdi mdi-eye"></i>
                                </a>
                            @endunless
                            @unless (auth()->user()->can('update', $billing))
                                @if ($billing->status != 'paid' && $billing->status != 'cancelled')
                                    <a x-on:click="$dispatch('set-billing-data-customer', { id: {{ $billing->id }} })" class="action-icon"
                                        role="button" data-bs-toggle="modal" data-bs-target="#edit-billing-image">
